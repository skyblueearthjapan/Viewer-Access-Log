...
**データ共有パス限定(2026-06-29)**: `(不明)` の正体＝SYSVOL/グループポリシー(`\\*\SYSVOL\…\Policies\`・Windows自動処理)＋パス無しイベント(全部署化で混入)。`AuditPgReader` direct/unknown 全4クエリに `position(chr(92)||'Data'||chr(92) in COALESCE(folder_path)||'|'||COALESCE(file_path))>0 OR …MTlock関連…` を追加→データ共有配下のパスのみ取り込み。(不明) 19,861→463(98%減)。残り: (不明)=Data共有ルート直下(部署サブ無し)／`system`=`D:\MTlock関連\system\_p2check\val_app`=VAL自身の配置ファイル(robocopyステージング痕跡)。**要 Reset-Cache**。
