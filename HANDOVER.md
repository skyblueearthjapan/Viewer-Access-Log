...
**P4b ⏳ 残り（規模:大）— 残りCRUD＋状態変更**
- `app.js` の **folders/users/rules/common_folders/user_folder_grants** タブは**まだ楽観更新モック(PROTO_MSG)**。実API化要：POST/PUT/DELETE エンドポイント追加（`monitored_folders`/`users`/`alert_rules`/`common_folders`/`user_folder_grants`）＋ ConfigWriter にメソッド追加（既存パターン踏襲）。
- **アラート/インシデント状態変更**: `PATCH /api/alerts/{id}/status`・`/api/incidents/{id}/status`（alert_histories.status / detected_incidents.status を config_editor で UPDATE＋監査）＋UIボタン。
- **ルール手動評価**: `POST /api/rules/{id}/evaluate`（AuditLogger側の評価をどう叩くか要設計）。
- ⚠️ 監視フォルダ/ルールの書込は**本番AuditLoggerの監視/検知挙動を変える**（約5分で反映）。慎重に。

> 大原則: データ(audit_logs/raw)は**読み取り専用を堅持**／**設定テーブルのみ** config_editor で書込。
