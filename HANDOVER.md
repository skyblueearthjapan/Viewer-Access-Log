...
## 0. TL;DR / 現在地（2026-06-30 時点）

- **Viewer-Access-Log（VAL）は本番稼働中**。MTSV上の Windows サービス `ViewerAccessLog`（自動起動・`D:\Apps\ViewerAccessLog`）。
  - URL: **http://lineworks-mtsv:5090**（社内ネットの管理者PCから閲覧・FW開放済・閲覧は匿名可/書込のみWindows認証）。
  - 役割: 共有フォルダへのアクセスを **🟦ビューアー経由 / 🟥サーバー直接 / ⬜未帰属** の3色で可視化し、NTFSロックダウンの「ビューアー矯正効果」を効果測定する **読み取り専用ダッシュボード**。
- **監視範囲＝全部署**（AuditLoggerと同等。技術部だけでなく D:\Data 配下全部＋MTlock関連）。集計はSQLite押し下げで**メモリ安全**。
- **画面**: ダッシュボード / ログ検索 / ユーザー別 / **📊部署別利用率** / アラート / 検知インシデント / サーバー状態 / **設定9タブ（読み書き）**。各ページは見出し＋操作部を固定(sticky)・本文のみスクロール。
- **データ品質対策（本日6/30 までに全部実装・本番反映）**:
  - 公平カウント＝**セッション数**(同一ユーザー×ファイル×10分バケットで畳む)＋ゴミ除外(`:Zone.Identifier`/desktop.ini/thumbs.db/*.lnk/*.tmp/~$)。
  - **データ共有パス限定**(Data\/MTlock関連\配下のみ)→SYSVOL/GPO/パス無しの(不明)を除去。`(不明)`(部署未解決=スラッシュのみ)と `MTlock関連\system`(VAL自身の配置)も取り込み除外。
  - **svc-dove/svc-zaiko等システム監視アカウントを全ソースから除外**。
  - **ビューアー部署を技術部に統一**(直接の技術部と合算可)、ユーザー表示は**設定の表示名**。
  - **Excelリンクの幻アクセス対策**(②段)＝バースト畳み込み(is_open)＋リンク先専用判定(file_link_targets)。「本人が開いていない他部署が出る」を解消。詳細は §4-4 と [[excel-link-phantom-reads]]。
- **実測の現実**: 技術部の利用率は約2〜3%(ビューアーは技術部専用ツール)、他部署は**ビューアー無しで0%＝全員直接**。＝ロックダウン前の素の状態。
- **残り（小／本丸）**: §4・§7 参照。**本丸＝技術部NTFSロックダウン本番**(未着手・利用者の現地操作とタイミング相談が必須)。
...
P1〜P3・P3仕上げ・P4a・P4b・全部署化・公平カウント・Excelリンク対策・UI固定 は**完了**。残りは：

1. **技術部ロックダウン本番**（本丸・未着手）: §5の手順。VALで効果測定しながら。前提=機械設計の残り10サブフォルダ MTSV$ 付与完了。**本番共有NTFS変更＝利用者の現地操作＋タイミング相談が必須**（[[do-not-touch-real-share]]）。現状ACLは [[gijutsubu-acl-state]]（Authenticated Users読取Allow残存＝非技術部が直読み可）。
2. **「総務部にURLを渡す」案（保留中の決定）**: 利用者が「このアプリのURLを総務部に渡したい」と希望。**未決の論点**=現ダッシュボードは全部署が見える＋閲覧ログイン無し。(A)そのまま渡す/(B)総務部専用に絞った版を別ポートで立てる/(C)閲覧にWindows認証を要求、のいずれか要選択（中断中）。
3. **P4ルール手動評価**（保留）: AuditLogger 評価エンジン連携の設計が要る。
4. 将来最適化: catalog.dbロック堅牢化（リトライ＋スナップショット）、viewerパスワード保管強化（Credential Manager 等）。

## 7-2. 気になる点・監視項目（次エージェントは必ず一読）

- **デプロイ手順（重要）**: コード変更後は ①ローカルで publish → ②`robocopy` で MTSV共有 `\\192.168.1.242\MTlock関連\system\_p2check\val_app` へステージ → ③`scripts/Update-ViewerAccessLog.ps1`（バイナリのみ・管理者資格情報のみ）。**スキーマ/スコープ/dept/is_open 等データ層を変えたら ④`scripts/Reset-ViewerAccessLogCache.ps1`（cache.db作り直し＝全部署バックフィル数分）が必須**。全 `!`+Get-Credential 方式。スクリプトは全て **ASCII限定**（PS5.1がBOM無し非ASCIIを誤読）。
- **`_p2check\val_app` の後始末**: これは**デプロイ用ステージング兼VAL配置元**。残すと監査ログに「自分たちの配置ファイル読取」が出る（現在は `MTlock関連\system` 除外で画面には出ない）。ロックダウン完了後など区切りで削除可（ただし削除すると次回 Update の前に再ステージが必要）。
- **MTSVコレクタの周期的 GAP / CATCHING_UP**: health に時々出るが、これは **AuditLogger 側の収集遅延**（VALの不具合ではない）。VALのGAP検出が正しく拾っているだけ。
- **Excelリンク対策の閾値**: `LiveOptions.BurstCollapseSeconds=3` / `LinkTargetMinReads=10` / `LinkTargetMaxOpenRatio=0.15`。リンク先判定は**読取10件以上溜まってから効く**ため Reset-Cache 直後は数分間まだ残って見える（正常）。効きが弱ければ ratio を上げる/秒数を広げる。
- **部署ラベルの軽微ノイズ**: 深い階層（`Data\<部署>` 直下でない構造）由来で `2014-09-13…JOB` 等が稀に部署として出る。実害小。
- **`lastSync` は実イベント時刻に追従**（同期停止バグは6/29修正済。もし lastSync が固着し UI 最新が数時間前なら、増分同期停止を疑い `scripts/Diag-SyncLag.ps1` で実DBと突き合わせ）。
- **診断スクリプト一覧（再対応に有用）**: `Diag-SyncLag`(同期遅延), `Diag-SourceCheck`(ソース判別), `Diag-PathAcl`(NTFS ACL), `Diag-ActionMix`(直接の内訳), `Diag-ExcelLinks`(リンク自動読取), `Diag-SettingsSchema`(設定テーブル実カラム), `Diag-PgPlan`(クエリ計画)。すべて読み取りのみ。
- **データのバックアップ**: VALの cache.db は**いつでも再生成可能**（元はAuditLogger PG＋SFE catalog.db）なのでバックアップ不要。**真のデータ源（audit_logger PG・catalog.db）はVALから読むだけで一切変更しない**。設定書込は config_editor 経由で `audit.config_audit_log` に監査記録が残る。
...
- **固定レイアウト(2026-06-30)**: `body` を 100vh 縦フレックス＋`overflow:hidden`、`header`(トップバー)/`nav`(サイドパネル)を固定、`main` だけスクロール。さらに各ページ先頭の「見出し＋操作部」を `.page-head`(sticky・負マージンでmainパディング打消し)で括り固定: ダッシュボード/部署別/ユーザー別=見出し＋期間バー、ログ検索=見出し＋期間＋ソースフィルタ＋検索条件カード、設定=見出し＋9タブ行、サーバー状態=見出し＋健全性バナー、一覧系(users/alerts/incidents)=見出し。本文(表/グラフ/タブ中身)のみスクロール。`app.js` の各ページ innerHTML と `styles.css` の `.page-head`。
...
**データ共有パス限定(2026-06-29)**: `(不明)` の正体＝SYSVOL/グループポリシー(`\\*\SYSVOL\…\Policies\`・Windows自動処理)＋パス無しイベント(全部署化で混入)。`AuditPgReader` direct/unknown 全4クエリに `position(chr(92)||'Data'||chr(92) in COALESCE(folder_path)||'|'||COALESCE(file_path))>0 OR …MTlock関連…` を追加→データ共有配下のパスのみ取り込み。(不明) 19,861→463(98%減)。**さらに(不明)/system を根本除外(2026-06-30)**: `FetchAuditRowsAsync` の行生成時に `dept=="(不明)"`(部署に解決しない=Data共有ルート直下・スラッシュのみ)行と `\MTlock関連\system\`配下(VAL自身の配置領域)を `continue` で取り込み除外。検証: (不明)0件・system0件・部署一覧は実部署のみ(技術部利用率2.9%)。**要 Reset-Cache**。

**Excelリンクの幻アクセス対策(2026-06-30)**: 直接アクセスは「ユーザーがExcelを開くと裏でリンク先ブックを自動読込」で水増し＝**本人が開いていない他部署が出る**(実証: 1秒で11ファイル横断・[[excel-link-phantom-reads]])。process_name未収集のため判別信号は時刻バーストのみ。2段で対策: ①**バースト畳み込み**(`SyncWorker`が取り込み時に `is_open` 算出—同一ユーザーの前読取から `LiveOptions.BurstCollapseSeconds`(3s)以内=リンク自動読み=0、それ以外=開いた操作=1。全SQLが is_open=1 のみ集計)。②**リンク先専用判定**(毎同期末に `CacheDb.RecomputeLinkTargets`→`file_link_targets`: direct系で読取>=`LinkTargetMinReads`(10)かつ is_open=1割合<`LinkTargetMaxOpenRatio`(0.15)=誰も単独で開かない。全SQLが `NOT EXISTS(file_link_targets)` で除外)。効果: 今泉のタイムラインから 生産工程表/担当太郎.xlsm/木場の箱 等の他部署リンク先が消え「実際に開いたファイル」のみに。ビューアー行は常に is_open=1。閾値は LiveOptions 可変。**判定は読取10件以上溜まってから効く→Reset-Cache後は数分で反映**。

**部署統一＋部署別利用率＋svc除外(2026-06-29)**: (A)ビューアー(`SfeSqliteSource`)のDeptを`opts.Dept`(技術部)固定→直接の技術部と合算可(表示パス不変)。(B)新 `GET /api/departments`＋画面「📊部署別利用率」(部署毎 viewer/direct/unknown/利用率・横バー・セッション集計・`LogService.Departments`/`CacheLogSource.DepartmentsSql`/`Models.DeptAdoption`)。(C)**svc-dove/svc-zaiko等システム監視アカウント完全除外**＝`AuditPgReader`の未帰属(unknown)クエリを `user_name IS NULL` のみに(MTSV$は二重計上防止で除外維持/svcは直接でも既存除外)＝どのソースにも出ない。検証: svc両0件・**技術部 viewer78/direct3423/利用率2.2%**(他部署はビューアー無で0%)。**dept変更のためデプロイ後 Reset-Cache 必須**。

**直接アクセスの公平カウント化(2026-06-29)**: 「ビューアー=意図的オープン1件／直接=フォルダ閲覧で中の全ファイルが件数化」の非対称を是正。実データで確認(action全部FILE_READ・`:Zone.Identifier`/Desktop.ini/Thumbs.db/.lnk/.tmp等ゴミ約19%・1ファイル3.15イベント反復)。①`AuditPgReader` direct/unknown でゴミ除外 ②`CacheLogSource` の集計を `COUNT(DISTINCT user|folder|file|substr(time,1,15)=10分バケット)`＝アクセス回数(セッション)に(両ソース公平)。**ログ検索/CSVは生イベントのまま**(詳細監査証跡)。効果:本日direct 生13,408→セッション5,600(約42%)。**注意**: 全部署スコープではビューアーは技術部専用ツールのため全社の利用率は低く出る→**技術部の利用率は dept=技術部 で絞って見る**。残課題: サムネイル等フォルダ閲覧由来の実ファイル読みは監査レベルで意図的オープンと分離不可。`(不明)`/`system` の部署ラベル残ノイズ少々。診断 `scripts/Diag-ActionMix.ps1`。

**全部署スコープ化＋SQLite押し下げ(2026-06-29)**: 利用者要望でVALの監視範囲を**AuditLoggerと同等(全部署=D:\Data配下全部＋MTlock関連・両サーバー)**に拡大。①`AuditPgReader`の技術部フィルタ除去＋dept抽出を共有ルート(Data\/MTlock関連\)直下の最初セグメントに修正 ②`CacheDb`のtimeをUTC固定幅で保存(範囲WHERE/並び正常化)＋idx_ar_dept ③**`LogService`をLive時SQLite押し下げ**(`CacheLogSource`にSearch/Summarize/Dashboard/Users/UserDetail/FiltersのSQLメソッド・全行メモリ保持を廃止＝RAM安全)。Sampleは従来LINQのまま。**デプロイ後 `scripts/Reset-ViewerAccessLogCache.ps1` でcache.db作り直し→全部署バックフィル必須**(time形式変更＋スコープ拡大のため)。検証:14日全部署dashboardが729msで応答(メモリ安全)・技術部/郵便局/営業部/総務部/購買部/TSC/製造部/部署間共通 が部署別に出現。**残ノイズ(任意クリーンアップ)**:Desktop.ini/Thumbs.db/*.lnk等システムファイルやMTlock関連直下ファイル・非標準パスが"部署"に混ざる。`LiveOptions.Dept`/`SyncWorker`のDept ログは残存(未使用・無害)。
