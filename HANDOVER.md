...
| 項目 | 現状（根拠） | 必要な実装 |
|---|---|---|
| **Live設定読取** | `CacheLogSource.Settings()` が**空を返す**（`CacheLogSource.cs` 189-190）→ **設定タブがLiveで空**。`AuditPgReader` に設定テーブル読取メソッド**欠落** | `AuditPgReader.ReadSettingsAsync()` 追加：`monitored_folders/users/alert_rules/detection_exclusions/common_folders/user_folder_grants/app_settings` をSELECT → `CacheLogSource.Settings()` から呼ぶ |
| **Live GAP検出** | `CacheLogSource.Gaps()` が**常に `[]`**（`CacheLogSource.cs` 110-111）→ 利用率のGAP除外がLiveで効かない | `collector_state`（`last_event_time`の途切れ）からGAP推測 `ReadGapsAsync()` |
| **catalog.db ロック堅牢化** | `SfeSqliteSource`(`Mode=ReadOnly`)失敗時リトライ無し・ログのみ（`SyncWorker.cs`）。SFE稼働中はロック競合の可能性 | 指数バックオフのリトライ＋スナップショット読取フォールバック |
